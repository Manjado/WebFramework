...
function (_super) {
  ...
  UserEdit.prototype.onRender = function () {
    ...
    new UserForm_1.UserForm(this.regions.userShow, this.model).render();
  };
...
function () {
  function Model(attributes, events, sync) {
    ...
    this.on = this.events.on; //  this notation can only be used if it is not present in the constructor
...
function () {
  ...
  ApiSync.prototype.save = function (data) {
    // sometimes TS has no idea what type T is going to be, solusion for that is using interface
    var id = data.id;
